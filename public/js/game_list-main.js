...
window.addEventListener('DOMContentLoaded', function () {
  ...
  getAllCategories().then(response => {
    ...
    for (let i = 0; i < response.length; i++) {
      let string = `<li class="cat-content first-cat" id="${response[i].id}"><a href="/admin_game_list?maincat=${encodeURI(response[i].category_name)}">${response[i].category_name}</a></li>`
      ...
      for (let j = 0; j < response[i].parent.length; j++) {
        string += `<li class="cat-content second-cat" id="1-${response[i].parent[j].fk_main}-${response[i].parent[j].id}"><a href="/admin_game_list?platform=${encodeURI(response[i].parent[j].category_name)}">${response[i].parent[j].category_name}</a></li>`
        ...
        for (let k = 0; k < response[i].parent[j].child.length; k++) {
          thirdstring += `<li><a href="/admin_game_list?childcat=${encodeURI(response[i].parent[j].child[k].category_name)}">${response[i].parent[j].child[k].category_name}</a></li>`
        }
        thirdstring += '</ul></div>'
...
function showGame () {
  ...
  window.history.pushState({
    page: 'same'
  }, 'same page', 'admin_game_list?' + params.toString())
